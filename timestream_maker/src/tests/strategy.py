...
def save_lst_lat(tod_file, lst, lat, spf):
    ...
    lat: array 
        the latitude timestream, for the center of the array. 
    spf: int
        the number of samples per frame
    Returns
    -------
...
if __name__ == "__main__":
    ...
    #We first save the TODs of pixels in the SW array, then in the LW array. 
    start = time.time()
    ...
    for array, freqs_of_array, pointing_paths_to_save, pixel_offset_array, pixel_shift_array in zip(
                                                           ...
                                                           (pointing_paths[:P['nb_pixel_SW']],  pointing_paths[P['nb_pixel_SW']:]),
                                                           (pixel_offset_SW, pixel_offset_LW),
                                                           (pixel_shift_SW, pixel_shift_LW)):
        ...
        for f, F in enumerate(freqs_of_array):
            ...
            if(array=='SW'): 
                names = det_names[f * P['nb_pixel_SW'] : (f + 1) * P['nb_pixel_SW']]
            else:
                index = P['nb_pixel_SW'] * P['nb_channels_per_array']
